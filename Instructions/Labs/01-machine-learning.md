---
lab:
  title: 探索 Azure 機器學習 中的自動化 機器學習
---

# 探索 Azure 機器學習 中的自動化 機器學習
...
1. 選取 **[+ 建立資源**]、搜尋 *機器學習*，並使用下列設定建立新的 **Azure 機器學習** 資源：
    - **訂用帳戶**：*您的 Azure 訂用帳戶*。
    - **資源群組**：建立或選取資源群組**。
    - **名稱**： *輸入工作區*的唯一名稱。
    - **區域**：選取最接近的地理區域**。
...
> **引文**： *本練習中使用的數據衍生自 [Capital Bikeshare](https://www.capitalbikeshare.com/system-data) ，且會根據已發佈的數據 [許可協定](https://www.capitalbikeshare.com/data-license-agreement)* 使用。

1. 在 [[Azure Machine Learning 工作室](https://ml.azure.com?azure-portal=true)] 中，檢視 [**自動化 ML**] 頁面 （在 [撰寫 **] 底下**）。
...
1. 使用下列設定建立新的自動化 ML 作業，並視需要使用 **Next** 來逐步執行使用者介面：
   ...
   **工作類型與資料**：
    ...
    - **選取資料集**：使用下列設定建立新的資料集：
        ...
        選取 **建立**。 建立數據集之後，選取 **自行車出租** 數據集以繼續提交自動化 ML 作業。
    ...
    **工作設定**：
    ...
    - **限制**： *展開本節*
        ...
        - **並行試用版**上限：3
        - **最大節點**數：3
        - **計量分數臨界值**：0.085（*因此，如果模型達到標準化根均方誤差度量分數為0.085或更少，作業就會結束。*
        - **逾時**：15
        - **反覆專案逾時**：15
        - **啟用早期終止**： *已選取*
    - **驗證與測試**：
...
1. 選取 [計量]**** 索引標籤，然後選取**殘差**與 **predicted_true** 圖表 (如果尚未選取)。 
    ...
    檢閱顯示模型效能的圖表。 **殘差圖表會將殘差***（預測值與實際值之間的差異）顯示為*直方圖。 **** predicted_true圖表會比較預測值與 true 值。 

## 部署和測試模型

1. 在 **[模型** ] 索引標籤上，針對自動化機器學習作業所定型的最佳模型，選取 **[部署** 並使用 **Web 服務** ] 選項，以下列設定來部署模型：
    - **名稱**：predict-rentals
    - **描述**：預測自行車租借
    - **計算類型**：Azure 容器執行個體
    - **啟用驗證**： *已選取*
...
1. 在 [Azure Machine Learning 工作室] 的左側功能表上，選取 **[端點]**，然後開啟**預測租用**即時端點。

1. 在預測 **租用** 即時端點頁面上，檢視 [ **測試** ] 索引卷標。

1. 在 [ **要測試端點** 的輸入資料] 窗格中，以下列輸入數據取代範本 JSON：
