---
lab:
  title: 探索 Azure AI 搜尋服務索引 （UI）
---

# 探索 Azure AI 搜尋服務索引 （UI）

想像一下您在全國咖啡連鎖店 Fourth Coffee 工作。 系統會要求您協助組建知識採礦解決方案，以便輕鬆搜尋與客戶體驗相關的深入解析。 您決定使用從客戶檢閱擷取的數據來建置 Azure AI 搜尋索引。  
...
1. 單擊 [ **+ 建立資源** ] 按鈕、搜尋 *Azure AI 搜尋*，並使用下列設定建立 **Azure AI 搜尋** 資源：

    - **訂用帳戶**：*您的 Azure 訂用帳戶*。
    - **資源群組**：*選取或建立具有唯一名稱的資源群組*。
    - **服務名稱**：唯一名稱**。
    - **位置**： *選擇任何可用的區域。如果在美國東部，請使用「美國東部 2」*。
    - **定價層**：基本

1. 選取 [檢閱 + 建立]****，然後在看見回應「驗證成功」**** 之後，選取 [建立]****。

1. 完成部署後，選取 [移至資源]****。 在 Azure AI 搜尋概觀頁面上，您可以新增索引、匯入數據，以及搜尋建立的索引。

### 建立 Azure AI 服務資源

您必須布 **建與 Azure AI 搜尋服務資源位於相同位置的 Azure AI 服務** 資源。 您的搜尋解決方案會使用該資源搭配 AI 產生的深入解析，擴充資料存放區中的資料。

1. 返回 Azure 入口網站的首頁。 按一下 **＋建立資源 **按鈕並搜尋  * Azure AI 服務 *。 選取**建立** **Azure AI 服務**方案。 系統會帶您前往建立 Azure AI 服務資源的頁面。 使用下列設定對其進行設定：
    - **訂用帳戶**：*您的 Azure 訂用帳戶*。
    - **資源群組**： *與您的 Azure AI 搜尋資源*相同的資源群組。
    - **區域**： *與您的 Azure AI 搜尋資源*相同的位置。
    - **名稱**︰唯一名稱**。
    - **定價層**:標準 S0
...
1. 搜尋*儲存體帳戶*，並使用下列設定建立**儲存體帳戶**資源：
    - **訂用帳戶**：*您的 Azure 訂用帳戶*。
    - **資源群組**： *與您的 Azure AI 搜尋服務和 Azure AI 服務資源*相同的資源群組。
    - **儲存體帳戶名稱**：唯一名稱**。
    - **位置**：選擇任何可用的位置**。
    - **效能**：標準
    - **備援**：本地備援儲存體 (LRS)
...
1. 在您建立 Azure 儲存體 帳戶的左側功能表窗格中，選取 **[組態**] （在 [設定 **] 底下**）。
...
1. 在新的瀏覽器索引標籤中[，從 `https://aka.ms/mslearn-coffee-reviews`下載壓縮的咖啡評論](https://aka.ms/mslearn-coffee-reviews)，並將檔案解壓縮到 *reviews* 資料夾。
...
在記憶體中有文件之後，您可以使用 Azure AI 搜尋從檔擷取見解。 Azure 入口網站提供*匯入資料精靈*。 透過此精靈，您可以針對支援的資料來源自動建立索引和索引子。 您將使用精靈來建立索引，並將搜尋檔從記憶體匯入 Azure AI 搜尋服務索引。

1. 在 Azure 入口網站 中，流覽至您的 Azure AI 搜尋資源。 在 [概觀]**** 頁面上，選取 [匯入資料]****。
...
1. 返回您的 Azure AI 搜尋資源頁面。 在左窗格的 [搜尋管理]** 底下**，選取 **[索引器**]。 選取新建立 **的咖啡索引器**。 稍等一下，然後選取 [&orarr; 重新整理]****，直到 [狀態]**** 表示成功。
...
2. 請注意，所選取的索引是您建立的 coffee-index**。 在選取的索引下方，將*檢視變更為 **JSON 檢視***。 

    ![搜尋總管的螢幕擷取畫面。](media/create-cognitive-search-solution/search-explorer-query.png)

在 [ **JSON 查詢編輯器]** 字段中，複製並貼上： 
```json
{
    "search": "*",
    "count": true
...
4. 現在讓我們依位置進行篩選。 在 [ **JSON 查詢編輯器]** 字段中，複製並貼上： 
```json
{
...
6. 現在讓我們依情緒進行篩選。 在 [ **JSON 查詢編輯器]** 字段中，複製並貼上： 
```json
{
...
4. 選取 [編輯]**** 查看針對 Azure 資料存放區中其中一份文件產生的 JSON。
...
這個簡單的搜尋索引只會 搜尋服務 Azure AI 的一些功能。 若要深入瞭解如何使用這項服務，請參閱 [Azure AI 搜尋服務 頁面](https://learn.microsoft.com/azure/search)。
