...
1. 在您建立的 Azure 儲存體 帳戶中，於左側功能表窗格中選取 **[組態**] （設定 底下****）。
...
1. 在 Azure AI 搜尋資源的 [概觀 **] 頁面底部**，選取 [**索引器] 索引標籤**。此索引標籤會顯示新建立**的咖啡索引器**。 稍等一下，然後選取 [&orarr; 重新整理]****，直到 [狀態]**** 表示成功。
...
這個簡單的搜尋索引只提供 Azure AI 搜尋服務 的一些功能。 若要深入瞭解如何使用這項服務，請參閱 [Azure AI 搜尋服務 頁面](/azure/search/search-what-is-azure-search)。
